...
const quoteTemplateStylesXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <fonts count="6">
    ...
    <font><b/><sz val="18"/><color rgb="FF111111"/><name val="Aptos Display"/></font>
    <font><b/><sz val="11"/><color rgb="FF111111"/><name val="Aptos"/></font>
    <font><b/><sz val="11"/><color rgb="FFFFFFFF"/><name val="Aptos"/></font>
    <font><b/><sz val="11"/><color rgb="FF111111"/><name val="Aptos"/></font>
    <font><sz val="10"/><color rgb="FF263B3F"/><name val="Aptos"/></font>
  </fonts>
  <fills count="5">
    <fill><patternFill patternType="none"/></fill>
    <fill><patternFill patternType="gray125"/></fill>
    <fill><patternFill patternType="solid"><fgColor rgb="FFE7EB48"/><bgColor indexed="64"/></patternFill></fill>
  ...
  <cellXfs count="11">
    ...
    <xf numFmtId="0" fontId="1" fillId="2" borderId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="left" vertical="center"/></xf>
    ...
    <xf numFmtId="0" fontId="2" fillId="3" borderId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="left" vertical="center"/></xf>
    <xf numFmtId="0" fontId="5" fillId="0" borderId="1" applyFont="1" applyBorder="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf>
    <xf numFmtId="165" fontId="5" fillId="0" borderId="1" applyFont="1" applyBorder="1" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf>
    <xf numFmtId="165" fontId="4" fillId="2" borderId="1" applyFont="1" applyFill="1" applyBorder="1" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf>
    <xf numFmtId="167" fontId="4" fillId="2" borderId="1" applyFont="1" applyFill="1" applyBorder="1" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf>
    ...
    <xf numFmtId="0" fontId="5" fillId="0" borderId="1" applyFont="1" applyBorder="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf>
    <xf numFmtId="166" fontId="4" fillId="0" borderId="1" applyFont="1" applyBorder="1" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf>
  </cellXfs>
</styleSheet>`;
...
const xlsxColName = n => {
  let out = '';
  let x = n;
  while (x > 0) { x -= 1; out = String.fromCharCode(65 + (x % 26)) + out; x = Math.floor(x / 26); }
  return out;
};

const quoteXlsxCell = (row, col, value, style = 4, numeric = false) => {
...
const quoteTemplateFiles = (sheetXml, logoBytes = null) => {
  const hasLogo = logoBytes instanceof Uint8Array && logoBytes.length > 0;
  const contentTypes = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/>${hasLogo ? '<Default Extension="png" ContentType="image/png"/><Override PartName="/xl/drawings/drawing1.xml" ContentType="application/vnd.openxmlformats-officedocument.drawing+xml"/>' : ''}<Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/><Override PartName="/docProps/core.xml" ContentType="application/vnd.openxmlformats-package.core-properties+xml"/><Override PartName="/docProps/app.xml" ContentType="application/vnd.openxmlformats-officedocument.extended-properties+xml"/></Types>`;
  const files = [
    ...
    { name: 'xl/workbook.xml', content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Cotización" sheetId="1" r:id="rId1"/></sheets></workbook>` },
    { name: 'xl/_rels/workbook.xml.rels', content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>` },
    { name: 'xl/styles.xml', content: quoteTemplateStylesXml },
  ...
  if (hasLogo) {
    files.push(
      ...
      { name: 'xl/drawings/drawing1.xml', content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><xdr:twoCellAnchor editAs="oneCell"><xdr:from><xdr:col>6</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>0</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from><xdr:to><xdr:col>8</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>2</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:to><xdr:pic><xdr:nvPicPr><xdr:cNvPr id="2" name="Logo FESTOS"/><xdr:cNvPicPr/></xdr:nvPicPr><xdr:blipFill><a:blip r:embed="rId1"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic><xdr:clientData/></xdr:twoCellAnchor></xdr:wsDr>` },
      { name: 'xl/drawings/_rels/drawing1.xml.rels', content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/festoslogo.png"/></Relationships>` },
      { name: 'xl/media/festoslogo.png', bytes: logoBytes }
    );
  }
  return files;
};
...
export function Proyectos({ usuario, onNotify, onAudit, puedeGestionar = true, filtroDesdeDashboard = null, solicitudNuevo = false, onConsumirNuevo = () => {}, voiceCommand = null, onVoiceFeedback = () => {} }) {
  ...
  const guardar = async e => {
    ...
    try {
      ...
      window.dispatchEvent(new Event('festos:proyectos-actualizados'));
...
export function Cotizaciones({ usuario, onNotify, onAudit, puedeAprobar = true, puedeCrearCliente = false, solicitudNuevo = false, onConsumirNuevo = () => {}, voiceCommand = null, onVoiceFeedback = () => {} }) {
  ...
  const reset = () => { setForm({ client_id: '', proyecto_nombre: '', project_id: '', lob: '', descripcion: '', ejecutivo: ejecutivoDefault, aplicar_igv: true, descuento: 0, modo: 'detallado', items: [newItem()] }); setValorVentaDirecto(0); setEditando(null); setItemsEditando(new Set()); setItemsColapsados(new Set()); setVista('lista'); };
  const nuevaCotizacion = () => { const item = newItem(); setEditando(null); setForm({ client_id: '', proyecto_nombre: '', project_id: '', lob: '', descripcion: '', ejecutivo: ejecutivoDefault, aplicar_igv: true, descuento: 0, modo: 'detallado', items: [item] }); setValorVentaDirecto(0); setItemsEditando(new Set([item.id])); setItemsColapsados(new Set()); setNuevoCliente(false); setVista('form'); };
  ...
  const descargarPDF = async (q) => {
    try {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(10);
      doc.text('VALOR VENTA', summaryX + 4, y + 2);
      doc.text(money(q.subtotal), W - margin - 4, y + 2, { align: 'right' });
      y += 19;
  ...
  const descargarExcel = async q => {
    ...
    try {
      ...
      pushRow(16, 20, [quoteXlsxCell(16, 1, 'IGV', 4), quoteXlsxCell(16, 3, num(q.igv), 5, true)]);
      pushRow(17, 20, [quoteXlsxCell(17, 1, 'Importe con IGV', 4), quoteXlsxCell(17, 3, num(q.total), 5, true)]);
      pushRow(18, 20, [quoteXlsxCell(18, 1, 'Costo estimado', 4), quoteXlsxCell(18, 3, num(q.costo_estimado), 5, true)]);
      pushRow(19, 20, [quoteXlsxCell(19, 1, 'Ganancia / Utilidad estimada', 4), quoteXlsxCell(19, 3, num(q.ganancia_estimada), 5, true)]);
      pushRow(20, 22, [quoteXlsxCell(20, 1, 'Margen global (%)', 4), quoteXlsxCell(20, 3, num(q.margen) / 100, 7, true)]);
      pushRow(21, 10, []);
      pushRow(22, 24, [
        quoteXlsxCell(22, 1, 'N°', 8), quoteXlsxCell(22, 2, 'Descripción', 8), quoteXlsxCell(22, 3, 'Cantidad', 8),
        quoteXlsxCell(22, 4, 'Valor unitario', 8), quoteXlsxCell(22, 5, 'Valor total', 8), quoteXlsxCell(22, 6, 'Costo unitario', 8),
      ...
      const merges = [
        ...
        'A14:H14', 'A15:B15', 'C15:D15', 'A16:B16', 'C16:D16', 'A17:B17', 'C17:D17', 'A18:B18', 'C18:D18', 'A19:B19', 'C19:D19', 'A20:B20', 'C20:D20'
      ];
  ...
  return <div className="quote-item quote-item-pro" key={item.id}>
    <div className="quote-item-top quote-item-pro-head">
      <div className="quote-item-number"><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{itemsColapsados.has(item.id) ? (item.descripcion?.trim() || `Ítem ${index + 1}`) : `Ítem ${index + 1}`}</strong><small>{itemsColapsados.has(item.id) ? `Ítem ${String(index + 1).padStart(2, '0')} · ${money(valorTotal)}` : 'Detalle de valorización'}</small></div></div>
