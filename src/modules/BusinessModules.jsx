...
export function Proyectos({ usuario, onNotify, onAudit, puedeGestionar = true, filtroDesdeDashboard = null, solicitudNuevo = false, onConsumirNuevo = () => {}, voiceCommand = null, onVoiceFeedback = () => {} }) {
  ...
  return <div onClick={() => menuAbierto && setMenuAbierto(null)}>
    ...
    <div className="glass-card form-card business-toolbar">
      ...
      <div className="project-filter-subtotal-row"><div className="project-filter-total"><span>Valor venta filtrado (sin IGV)</span><strong>{money(subtotalFiltrado)}</strong></div></div>
    </div>
    <div className="business-record-list">
  ...
  const descargarExcel = async q => {
    ...
    try {
      ...
      const detailRows = [
        ...
        [{ value: 'Valor venta (sin IGV)', style: 2 }, { value: num(q.subtotal), numeric: true, style: 7 }, { value: 'Descuento', style: 2 }, { value: num(q.descuento), numeric: true, style: 7 }],
        [{ value: 'IGV', style: 2 }, { value: num(q.igv), numeric: true, style: 7 }, { value: 'Importe con IGV', style: 2 }, { value: num(q.total), numeric: true, style: 7 }],
