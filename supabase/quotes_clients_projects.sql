-- CONTROL FESTOS · Clientes / Proyectos / Cotizaciones
-- Ejecutar una sola vez en Supabase SQL Editor.
-- Mantiene pagos/facturas existentes sin modificarlos.
...
create table if not exists public.cotizaciones (
  ...
  client_id uuid not null references public.clientes(id) on update cascade on delete restrict,
  project_id uuid not null references public.proyectos(id) on update cascade on delete restrict,
  descripcion text,
...
create trigger trg_clientes_updated_at before update on public.clientes for each row execute function public.set_festos_updated_at();
...
-- IMPORTANTE: la aplicación actual usa un login propio en el frontend y no Supabase Auth.
-- Por compatibilidad, estas políticas permiten las operaciones desde el cliente anon.
-- Cuando migremos el login a Supabase Auth, reemplazar estas políticas por reglas basadas en auth.uid().
drop policy if exists "festos clientes anon access" on public.clientes;
create policy "festos clientes anon access" on public.clientes for all to anon, authenticated using (true) with check (true);
